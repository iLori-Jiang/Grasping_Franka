...
def detect_ar_marker(color_frame, intr_matrix, dist_coeff):
    ...
    marker_corners, marker_ids, rejected_candidates = cv.aruco.detectMarkers(color_frame, dictionary,
                                                                             parameters=parameters,
                                                                             cameraMatrix=intr_matrix,
...
if __name__ == '__main__':
    ...
    while (True):
        ...
        if (len(corners) > 0):
            R, t, _ = cv.aruco.estimatePoseSingleMarkers(corners, 0.05, cam.get_intrinsics_matrix(),
            ...
            visualize_img = cv.aruco.drawAxis(visualize_img, cam.get_intrinsics_matrix(), cam.get_distortion_coeffs(),
                                              R, t, 0.03)
