...
def pick_place(robot_server: ArmController, gripper_server: object, home_joint, pick_xyzrt, pick_xyzrt_up, place_xyzrt):
    ...
    time.sleep(0.1)
    # go to pick
    pick_xyzrt[2] = pick_xyzrt[2] - 0.13
...
if __name__ == '__main__':
    ...
    robot.move_j(home_joints, 0.8, 0.8)
    ...
    while 1:
        ...
        ret, uv, cla, cfi = detectObject(object_detector, color=color, crop_bounding=crop_bounding)    # [100, 347, 171, 460]
        if not ret:
            continue
        ...
        # Todo: filter plane
        plane_model = [-0.03110741,  0.06798567,  0.63743121, -0.76687139]
        new_pc = []
